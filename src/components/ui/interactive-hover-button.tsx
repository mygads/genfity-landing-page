...
>(({ text = "Button", className, link, ...props }, ref) => {
  return (
    <Link href={link} className={cn("relative w-28 cursor-pointer", className)}>
      <div className="group relative overflow-hidden rounded-xl bg-background p-x-2 py-1 text-center font-semibold border border-primary dark:border-primary-dark">
        ...
        <div className="absolute top-0 z-10 flex h-full w-full translate-x-6 items-center justify-center gap-2 text-sm text-primary-foreground opacity-0 transition-all duration-300 group-hover:-translate-x-0 group-hover:opacity-100 dark:bg-primary-dark dark:text-black text-white">
          <span>{text}</span>
          <ArrowRight size={18}/>
        </div>
